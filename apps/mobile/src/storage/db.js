...
import { primeRoleImages } from '../data/images';
import { ensureRemoteDailyTasks, fetchRemoteRoles, fetchRoleSeedMessages } from '../services/cloud';
...
async function seedInitialData() {
  ...
  try {
    ...
    if (Array.isArray(remoteRoles) && remoteRoles.length) {
      primeRoleImages(remoteRoles);
    }
  } catch (error) {
    console.warn('[DB] fetchRemoteRoles failed, fallback to local seeds', error);
  }
  try {
    seedMessages = await fetchRoleSeedMessages();
  } catch (error) {
    console.warn('[DB] fetchRoleSeedMessages failed, fallback to local seeds', error);
  }

  const messageMap = seedMessages.reduce((acc, msg) => {
    if (!msg?.roleId) return acc;
    if (!acc[msg.roleId]) acc[msg.roleId] = [];
    acc[msg.roleId].push(msg);
    return acc;
  }, {});
  Object.values(messageMap).forEach((list) => list.sort((a, b) => (a.position ?? 0) - (b.position ?? 0)));

  const rolesToSeed = remoteRoles.length ? remoteRoles : roleSeeds;

  // Always refresh core seed roles from cloud to keep data in sync
  ...
  console.log(`[DB] Seeding roles from ${remoteRoles.length ? 'Supabase' : 'local seeds'} (${rolesToSeed.length})`);

  primeRoleImages(rolesToSeed);

  for (const role of rolesToSeed) {
    const conversationId = role.conversation?.id || `${role.id}-default`;
    const initialMessages =
      messageMap[role.id] ||
      role.conversation?.initialMessages ||
      [];
    ...
    for (const message of initialMessages) {
      await run(
        ...
        [conversationId, message.sender, message.body, message.createdAt || now()]
      );
    }
  }
...
async function replaceLocalDailyTasks(dayKey, tasks = []) {
  ...
  for (const task of tasks) {
    await run(
      ...
      [
        task.id || uniqueId('remote-task'),
        task.title,
        task.description,
        task.scene,
        task.target_role_id || task.targetRoleId,
        task.kickoff_prompt || task.kickoff,
        task.difficulty || 'M',
        typeof task.target_words === 'string' ? task.target_words : JSON.stringify(task.target_words || []),
        task.reward_points || 5,
        task.reward_coins || getTaskCoinReward(task.reward_points || 5),
        task.completed ? 1 : 0,
        task.day_key || dayKey,
        task.created_at || now(),
        task.updated_at || now(),
...
export async function ensureDailyTasksForDay(dayKey) {
  try {
    const remoteTasks = await ensureRemoteDailyTasks(dayKey);
    if (remoteTasks && remoteTasks.length) {
      await replaceLocalDailyTasks(dayKey, remoteTasks);
      return remoteTasks;
    }
  } catch (error) {
    console.warn('[DailyTheater] remote sync failed, fallback to local pool', error);
  }
