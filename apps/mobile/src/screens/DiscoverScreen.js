...
import { loadExplorePosts } from '../data/explorePosts';
...
export default function DiscoverScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    loadExplorePosts()
      .then((items) => {
        if (mounted) setExploreItems(items || []);
      })
      .catch((error) => {
        console.warn('[Discover] loadExplorePosts failed', error);
        if (mounted) setExploreItems([]);
      })
      .finally(() => {
  ...
  const loading = loadingRoles || loadingContent;
  ...
  const contentCards = useMemo(() => exploreItems, [exploreItems]);
