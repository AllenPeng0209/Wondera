...
export default function ExplorePostScreen({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    async function fetchPost() {
      try {
        const result = await getExplorePostById(postId);
        if (mounted) setPost(result);
      } catch (error) {
        console.warn('[ExplorePostScreen] fetch failed', error);
      } finally {
      ...
      <View style={styles.screen}>
        ...
        <SafeAreaView style={[styles.safeArea, { paddingTop: topPadding }]}>
          <View style={styles.loadingState}>
            ...
            <Text style={styles.emptySubtitle}>鍐呭鍔犺浇涓€?..</Text>
          </View>
        </SafeAreaView>
      </View>
    );
  ...
  const images = useMemo(() => {
    if (Array.isArray(post.images) && post.images.length) return post.images.map(resolveSource).filter(Boolean);
    if (coverImage) return [resolveSource(coverImage)].filter(Boolean);
    return [];
        ...
        >
          ...
          <View style={styles.contentCard}>
            <View style={styles.authorRow}>
              {post.author?.avatar ? (
                <Image source={resolveSource(post.author.avatar)} style={styles.authorAvatar} />
              ) : null}
