...
export default function WorldExploreScreen({ navigation, route }) {
  ...
  const [world, setWorld] = useState(initialWorld || null);
  const [loading, setLoading] = useState(!initialWorld);
  useEffect(() => {
    let mounted = true;
    if (initialWorld) return undefined;
    async function loadWorld() {
      try {
        const item = await getExploreWorldById(worldId);
        if (mounted) setWorld(item);
      } catch (error) {
        console.warn('[WorldExplore] load failed', error);
      } finally {
      ...
      <View style={styles.screen}>
        ...
        <SafeAreaView style={[styles.safeArea, { paddingTop: topPadding }]}>
          <View style={styles.emptyState}>
            <ActivityIndicator color="#5b7aa6" />
            <Text style={styles.emptyTitle}>涓栫晫鎺㈢储鍔犺浇涓€?..</Text>
          </View>
        </SafeAreaView>
      </View>
    );
