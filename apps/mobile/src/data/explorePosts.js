...
import { getCachedExploreItemById, getCachedExploreItems, loadExploreCache } from '../services/cloud';
...
export async function loadExplorePosts(force = false) {
  try {
    const remote = await loadExploreCache(force);
    if (remote && remote.length) return remote;
  } catch (error) {
    console.warn('[Explore] load from Supabase failed, fallback to local seeds', error);
  }
  return explorePosts;
}

export function getCachedExplorePosts() {
  const remote = getCachedExploreItems();
  if (remote && remote.length) return remote;
  return explorePosts;
}

export async function getExplorePostById(id) {
  if (!id) return null;
  const cached = getCachedExploreItemById(id);
  if (cached && cached.type === 'post') return cached;
  const list = await loadExplorePosts();
  return list.find((post) => post.id === id && post.type === 'post') || null;
}

export async function getExploreWorldById(id) {
  if (!id) return null;
  const cached = getCachedExploreItemById(id);
  if (cached && cached.type === 'world') return cached;
  const list = await loadExplorePosts();
  return list.find((post) => post.id === id && post.type === 'world') || null;
}
